import os
import torch
import torch.optim as optim
import pandas as pd
from src.model import WaveletAutoEncoder
from src.utils import load_data, preprocess_data2
from src.trainer import train, evaluate, init_optimizer
from src.config import build_parser
from collections import OrderedDict
import numpy as np

# Model configuration dictionary
model_config = {
    'batchsize': 32,
    'epochs': 200,
    'learning_rate': 1e-5,
    'weight_decay': 1e-5,
    'encoder_layers': 2,
    'encoder_hidden_dim': 256,
    'fusion_dim': 256,
    'bottleneck_dim': 32,
    'random_seed': 42,
    'device': 'cuda:0',
    'k': 5,
    'wavelet_degree': 2,
    'tao': 0.1
}

# Load configuration
parser = build_parser()
c = parser.parse_args()
print(f"Type of config object: {type(c)}")

np.random.seed(model_config['random_seed'])
torch.manual_seed(model_config['random_seed'])
torch.cuda.manual_seed(model_config['random_seed'])
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

data_dir = "data"
data_files = [f for f in os.listdir(data_dir) if f.endswith(('.npz', '.csv', '.mat'))]

# To store the best result of each dataset
best_results = []

# Grid search ranges
k_values = list(range(5, 30, 5))
wavelet_degrees = list(range(2, 10, 2))
lr = [0.1, 0.01, 0.001, 0.0001]
batchsize_values = [128, 256, 512]

# Loop over all datasets
for dataset_name in data_files:
    print(f"Processing dataset: {dataset_name}...")

    X, y = load_data(os.path.join(data_dir, dataset_name))
    print(f"Data type of X: {X.dtype}")

    intermediate_results = []

    for wavelet_degree in wavelet_degrees:
        model_config['wavelet_degree'] = wavelet_degree
        for k in k_values:
            model_config['k'] = k
            print(f"Current k: {k}")
            for batchsize in batchsize_values:
                model_config['batchsize'] = batchsize
                train_loader, test_loader = preprocess_data2((X, y), model_config)

                for learn_rate in lr:
                    c.exp_lr = learn_rate
                    print(f"Searching hyperparameters: k={k}, wavelet_degree={wavelet_degree}, batchsize={batchsize}, lr={learn_rate}...")

                    print("Initializing model...")
                    model = WaveletAutoEncoder(
                        input_dim=X.shape[1],
                        encoder_hidden_dim=model_config['encoder_hidden_dim'],
                        encoder_layers=model_config['encoder_layers'],
                        fusion_dim=model_config['fusion_dim'],
                        bottleneck_dim=model_config['bottleneck_dim'],
                        freq_num=model_config['wavelet_degree'] + 1,
                        wavelet_degree=model_config['wavelet_degree'],
                    ).to(device)

                    optimizer = init_optimizer(c=c, model_parameters=model.parameters(), device=device)
                    optimizer._optimizer_step_pre_hooks = OrderedDict()
                    optimizer._optimizer_step_post_hooks = OrderedDict()
                    print(f'Initialized optimizer: "{c.exp_optimizer}".')

                    print(f"Training model on dataset: {dataset_name}...")
                    train(model, optimizer, train_loader, model_config, c)

                    print(f"Evaluating model on dataset: {dataset_name}...")
                    auc, aupr, f1 = evaluate(test_loader, model_config, c)

                    intermediate_results.append({
                        'k': k,
                        'wavelet_degree': wavelet_degree,
                        'batchsize': batchsize,
                        'AUC': auc,
                        'AUPR': aupr,
                        'f1': f1,
                        'lr': learn_rate
                    })

                    results = pd.DataFrame(intermediate_results)
                    os.makedirs("result", exist_ok=True)
                    results.to_csv(f"result/{dataset_name}_results.csv", index=False)

                    print(f"Dataset: {dataset_name}, k={k}, wavelet_degree={wavelet_degree}, AUC: {auc:.4f}, AUPR: {aupr:.4f}")

    best_result = max(intermediate_results, key=lambda x: x['AUPR'])

    best_results.append({
        'Dataset': dataset_name,
        'Best_k': best_result['k'],
        'Best_wavelet_degree': best_result['wavelet_degree'],
        'AUC': best_result['AUC'],
        'AUPR': best_result['AUPR'],
        'f1': best_result['f1'],
        'lr': best_result['lr']
    })

best_results_df = pd.DataFrame(best_results)
best_results_df.to_csv("result/best_result.csv", index=False)

print("All datasets processed. Best results saved to result/best_result.csv")
